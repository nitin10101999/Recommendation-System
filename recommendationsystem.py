...
def fetchData(filename):
	...
	sub_df.drop_duplicates(inplace = True)		# Remove duplicate 
	sub_df.reset_index(inplace = True,drop = True)
	sub_df.to_csv('sub_data.csv')
	return sub_df


""" divide data to use in the models """
def divideData(df,hotels_dic,authors_dic):	
	...
	for index in range(len(df)):	
		...
		arr = lst[3:10]
		for n, i in enumerate(arr):		# replace criteria rate = -1 to overall rate for the particualar rating
			if i == -1:
				arr[n] = lst[2]
		criteria_rates.append(arr)
	
	m = np.mean(criteria_rates)		# calculate mean
	s = np.std(criteria_rates)		# calculate standard deviation
	criteria_rates = np.subtract(criteria_rates,m)
	criteria_rates = np.divide(criteria_rates,s)	# Nomalize the criteria rates dataset
	
	return authorId_Evec, movieId_Evec, criteria_rates, overall_rates
	
""" Embedded layer to conver one hot encoding to low dimensional vector """
...
def model_criteria(input_dim):	
	model = Sequential()
	model.add(Dense(128,activation = 'relu',kernel_initializer = initializer ,input_shape = (input_dim,)))
...
""" map userId and hotelId features onehot encoding to embedded vector """
...
unique_authors = df.authors.unique()	# find unique authors
unique_hotels = df.hotels.unique()		# find unique hotels
...
# Save
np.save('hotels_dic64.npy', hotels_dic) 
np.save('authors_dic64.npy', authors_dic) 
"""
# Load
hotels_dic = np.load('hotels_dic64.npy',allow_pickle='TRUE').item()
authors_dic = np.load('authors_dic64.npy',allow_pickle='TRUE').item()
...
history_criteria = criteria_model.fit(np.asarray(concat_vec_id), np.asarray(criteria_rates), batch_size = 512, epochs = 20,validation_split = 0.2) # Train criteria rates model
print('overall Model --------------------------------------------------')
history_overall_rate = overall_rate_model.fit(np.asarray(criteria_rates), np.asarray(overall_rates), batch_size = 512, epochs = 20,validation_split = 0.2) # Train overall rate model
